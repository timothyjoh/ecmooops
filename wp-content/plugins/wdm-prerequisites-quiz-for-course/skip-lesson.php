		...
		foreach ( $post_args as $key => $val ) {
			...
			if ( $post_args[ $key ][ 'slug_name' ] == 'lessons' ) { // if lesson page
				...
				$post_args[ $key ][ 'fields' ][ 'wdm_skip_lesson' ] = array(
					...
					'help_text'	 => 'Select this to skip a lesson and navigate to a topic, quiz, or grassblade package. This will force a redirect from the lesson page. Code is in <b>wdm-prerequisites-quiz-for-course</b>.'
				);
	...
	if ( ! empty( $post_id ) ) {
		...
		if ( $wdm_lesson_skip_meta == 'on' ) {
			if ( get_post_type( $post_id ) == 'sfwd-lessons' ) { // if current post type is a lesson
				...
				$xapi = get_post_meta( $post_id, 'show_xapi_content', true );
				...
				if ( ! empty( $topics ) ) {
					...
					$url = do_shortcode("[grassblade text='$button_label' id=".$xapi." target='url']");
					wp_redirect( $url );
				}
			}
		}
