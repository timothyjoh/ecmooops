	...
	class Learndash_Admin_Groups_Users_List_Table extends WP_List_Table {
		...
		function column_group_actions( $item ) {
			...
			if ( ( !empty( $data_settings_courses ) ) && ( !empty( $data_settings_quizzes ) ) ) {
				...
				$data_slug = 'user-courses';
				$actions[$data_slug] = '<a href="'. admin_url( 'edit.php?post_type=sfwd-courses&page=group_admin_page&action=sfp_update_module&nonce-sfwd=' . wp_create_nonce( 'sfwd-nonce' ) . '&page_options=sfp_home_description&courses_export_submit=Export&group_id=' . $item->ID ) .'">'. __( 'Export Progress', 'learndash' ) .'</a>';
				
				$data_slug = 'user-quizzes';
				$actions[$data_slug] = '<a href="'. admin_url( 'edit.php?post_type=sfwd-courses&page=group_admin_page&action=sfp_update_module&nonce-sfwd=' . wp_create_nonce( 'sfwd-nonce' ) . '&page_options=sfp_home_description&quiz_export_submit=Export&group_id=' . $item->ID ) .'">'. __( 'Export Results', 'learndash' ) .'</a>';
