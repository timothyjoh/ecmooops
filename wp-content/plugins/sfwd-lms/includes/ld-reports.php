 ...
 *
 * @param  string/array $query_args usd to override default query args
 *		'exclude' bool operator on 'roles' parameter.
 * 		'roles' string comma or array of roles to include/exclude
 * @param  string/array $return_field used to specify the field to return. 
 *		Default is blank and will return the count (integer) of users. 
 * 		If vlaue is 'avail_roles' an array will be returned with counts per element.
 *
 * @uses This function used the WordPress function count_users()
 *
 * @return int/array Returns count of users (default. Or can return array of roles with counts
 * 
 * @since 2.3
 */
function learndash_students_enrolled_count( $query_args = array(), $return_field = '' ) {
	...
	$default_args = array(
		'exclude'	=>	true,
		'roles'		=>	array('administrator', 'none')
	);
	
	foreach( $default_args as $d_key => $d_val ) {
		if ( is_array( $default_args[$d_key] ) ) {
			if ( ( isset( $query_args[$d_key] ) ) && ( !empty( $query_args[$d_key] ) ) ) {
				if ( !is_array( $query_args[$d_key] ) ) {
					$query_args[$d_key] = array_map( 'trim', explode(',', $query_args[$d_key] ) );
				}
				
				if ( ( is_array( $query_args[$d_key] ) ) && ( !empty( $query_args[$d_key] ) ) ) {
					$query_args[$d_key] = array_unique( $query_args[$d_key] );
				}
			}
		} else {
			if ( ( !isset( $query_args[$d_key] ) ) || ( empty( $query_args[$d_key] ) ) ) {
				$query_args[$d_key] = $default_args[$d_key];
			}
		}
	}
	...
	if ( ( isset( $users['avail_roles'] ) ) && ( !empty( $users['avail_roles'] ) ) ) {
		if ( !empty( $query_args['roles'] ) ) {
			
			foreach( $users['avail_roles'] as $role => $count ) {
				if ($query_args['exclude'] == true) {
					if ( !in_array( $role, $query_args['roles'] ) ) {
						unset($users['avail_roles'][$role]);
					}
				} else if ($query_args['exclude'] != true) {
					if ( in_array( $role, $query_args['roles'] ) ) {
						unset($users['avail_roles'][$role]);
					}
				}
		...
		if ( ( $return_field == 'avail_roles') && ( isset( $users['avail_roles'] ) ) ) { 
			if ( !empty( $users['avail_roles'] ) ) {
				$return = $users['avail_roles'];
			}
		} else {
			if ( !empty( $users['avail_roles'] ) ) {
				foreach( $users['avail_roles'] as $role => $count ) {
	...
	} else if ( ( isset( $users['total_users'] ) ) && ( !empty( $users['total_users'] ) ) ) {
...
function learndash_reports_get_activity( $query_args = array(), $current_user_id = 0 ) {
	...
	$sql_str_tables = " FROM ". $wpdb->prefix ."users as users ";
...
//	$sql_str_joins  .= " LEFT JOIN ". $wpdb->prefix ."learndash_user_activity_meta as ld_user_activity_meta_steps_completed ON ld_user_activity_meta_steps_completed.activity_id=ld_user_activity.activity_id AND ld_user_activity_meta_steps_completed.activity_meta_key='steps_completed' \r\n";
	...
	if ( $ACTIVITY_STATUS_HAS_NULL !== true ) {
		...
		if ( !empty( $query_args['activity_status'] ) ) {
			...
			$sql_str_where .= " AND ld_user_activity.activity_status IS NULL ";
		}
